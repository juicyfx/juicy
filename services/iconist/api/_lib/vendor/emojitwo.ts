...
export async function generate(req: GenerateRequest): Promise<string> {
  try {
    // Read icon file
    var file = await readFile(path.resolve('node_modules', `@obr/emojitwo/dist/${req.icon}.svg`));
  } catch (e) {
    throw new NotFoundError(Vendor.emojitwo, req.icon);
  }
